...
function StatCards({ apartment }: StatCardsProps) {
  ...
  return (
    <div className="flex items-stretch gap-2">
      {hasPair ? (
        <div className="flex shrink-0 items-stretch rounded-[20px] bg-surface-sunken">
          {beds !== undefined ? (
            <StatItem
              glyph={BedroomIcon}
...
function SingleStat({ glyph, label }: StatItemProps) {
  return (
    <div className="flex w-[135px] shrink-0 flex-col items-start justify-center gap-1.5 rounded-[20px] bg-surface-sunken p-4">
      <Icon glyph={glyph} size={20} className="text-secondary" />
